...
def save_survey_result(slack_user_id: str, db: Session = Depends(get_db)):
    try:
        ...
        【出力における条件】
        - 出力の冒頭ではまず初めに参照している最新のアンケート回答がいつ作成されたものかを明記してください。
        （例: [※最新の回答登録日: 2024年8月17日]
        - 全ての項目において、文章を構造化し、内容は長い文章ではなく箇条書きや改行を活用して出力すること。
        - 「超多忙な人が1分でぱっと読んでアンケート回答の内容を把握できる」文章に仕上げること。
        - アンケートの回答者の性別が何であっても差し支えのない表現を用いること（彼、彼女は使用しない等）
        - アンケートの回答から読み取れる「事実」と、回答から考えられる「推測」や「飛躍」が混同しないように留意してください。
        - 出力フォーマットで <>の中にセクションを設けていますが、ニュアンスが大きくずれなければ、表現や文言を調整、変更することを許容します。
        """
        ...
        response = client.chat.completions.create(
            ...
            model="gpt-4o-mini",
            max_tokens=5000,
            temperature=0.00001 # 変更して試してみるのが良さそう。
        )
